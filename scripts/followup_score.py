...
SCORE_VERSION = "classifiability_v0_initial"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute initial follow-up classifiability scores."
    )
    parser.add_argument(
        "--db",
        required=True,
...
def ensure_tables_exist(con: sqlite3.Connection) -> None:
    ...
    rows = cur.execute(
        ...
        FROM sqlite_master
        WHERE type='table'
          AND name IN ('followup_queue', 'followup_score_history', 'decisions')
        """
    ).fetchall()
...
def fetch_latest_passed_decision(
    ...
    # Fallback: latest passed for the same object_id
    row = cur.execute(
        """
        SELECT object_id, candid, topic, passed, reason, metrics_json, created_utc
        FROM decisions
...
def evolution_proxy_score(ndethist: int | None) -> float:
    # Initial proxy only. Real evolution will come later from multi-epoch history.
    if ndethist is None:
        return 5.0
    if ndethist <= 1:
        return 15.0
...
def observability_placeholder_score() -> float:
    # Placeholder neutral score for v0.
    # Real sky visibility should be computed later from coordinates + site + time.
    return 7.5
...
def hostless_cleanliness_score(
    distpsnr1: float | None,
    srmag1: float | None,
) -> float:
    # srmag1 = -999 often means "no useful nearby PS1 counterpart"
...
def external_status_score(
    external_classification: int | None,
    tns_name: str | None,
) -> float:
...
def update_followup_queue(
    ...
    next_review_utc = (
        ...
        + (timedelta(hours=6) if total_score >= 60.0 else timedelta(hours=12))
...
def main() -> int:
    ...
    try:
        ...
        for queue_row in queue_rows:
            decision_row = fetch_latest_passed_decision(
                con,
                queue_row["object_id"],
                queue_row["candid"],
            )
            ...
            score = compute_score(queue_row, decision_row)
            scored += 1

            preview_lines.append(
                f"SCORE object_id={queue_row['object_id']} candid={queue_row['candid']} "
                f"total={score['total_score']:.1f} mag={score['mag']} "
                f"days_since_nondet={score['days_since_nondet']} "
                f"nmtchps={score['nmtchps']} distpsnr1={score['distpsnr1']} srmag1={score['srmag1']}"
            )
        ...
        cur = con.cursor()
        hist_count = cur.execute(
            "SELECT COUNT(*) FROM followup_score_history"
        ).fetchone()[0]
